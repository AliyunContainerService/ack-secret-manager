...
func getClientName(provider, name, namespace string, data api.DataSource) string {
	...
	clientName := fmt.Sprintf("%s/%s/%s", namespace, name, provider)

	if data.KmsEndpoint != "" {
		clientName = fmt.Sprintf("%s/%s", clientName, data.KmsEndpoint)
	}

	if data.SecretStoreRef != nil {
		clientName = fmt.Sprintf("%s/%s/%s", clientName, data.SecretStoreRef.Name, data.SecretStoreRef.Namespace)
	}
	...
	for _, data := range externalSec.Spec.Data {
		...
		if _, err := provider.GetClient(clientName); err != nil {
			...
			if data.SecretStoreRef != nil {
				...
				if err != nil {
					errors = append(errors, fmt.Errorf("get secret store %v error %v", data.SecretStoreRef.Name, err))
					continue
				}
			}
			...
			if err != nil {
				errors = append(errors, fmt.Errorf("could not new provider client %s", clientName))
			}
