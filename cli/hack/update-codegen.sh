...
# that deepcopy-gen can operate in
	...
	fake_repopath=$fake_gopath/src/github.com/AliyunContainerService/ack-secret-manager/cli
...
if [[ "$(realpath ${valid_gopath}/src/github.com/AliyunContainerService/ack-secret-manager/cli)" == "${REPO_FULL_PATH}" ]]; then
...
GOPATH="${temp_gopath}" GOFLAGS="" bash ${CODEGEN_PKG}/generate-groups.sh "client,informer,lister" \
	github.com/AliyunContainerService/ack-secret-manager/cli/pkg/client \
	github.com/AliyunContainerService/ack-secret-manager/cli/pkg/apis \
