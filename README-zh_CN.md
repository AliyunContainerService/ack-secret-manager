...
| ----------- | ----------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| 专属网关     | KMS私网域名 | {kms-instance-id}.cryptoservice.kms.aliyuncs.com | 1. 要求 KMS 实例和集群 Region 相同且VPC 相同<br />2. 替换 {kms-instance-id} 为实际的 KMS 实例 ID<br />3. KMS 实例版本 3.0 以上 |
| 共享网关     | VPC域名     | kms-vpc.{region}.aliyuncs.com                    | 1. 要求 KMS 实例和集群 Region 相同<br />2. 替换 {region} 为 KMS 实例所在的region<br />3. 应用默认配置，使用此地址时无需配置     |
| 共享网关     | 公网        | kms.{region}.aliyuncs.com                        | 1. 替换 {region} 为 KMS 实例所在的 region<br />2. 集群具有公网访问能力                                                       |
...
| `0.5.9` | 2025年5月29日  | 支持使用ExternalSecret配置KMS实例维度的KMS Service Endpoint对接多KMS实例                                       |
