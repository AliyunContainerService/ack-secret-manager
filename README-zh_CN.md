...
ack-secret-manager 包含两种 CRD，其中SecretStore 用于存放访问凭据（例如 RRSA ，ClientKey，AK 配置等），ExternalSecret 用于存放需要同步的凭据基础信息（如凭据名称，版本等）以及指定 SecretStore，保证了权限与数据分离，增强使用灵活性。具体介绍 [CRD参数配置指导](doc/crd.md)
...
2. 创建SecretStore & ExternalSecret
   ...
   创建名称为`hello-service-external-secret.yml`的ExternalSecret的测试模版文件，测试模板如下：
   ...
   另外ack-secret-manager支持跨账号同步凭据，在 `SecretStore.Spec.KMS.KMSAuth` 中配置 `remoteRamRoleArn`，`remoteRamRoleSessionName` 即可，以下为样例 SecretStore：
