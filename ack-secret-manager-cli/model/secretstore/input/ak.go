...
func init() {
	m := initialAKModel()
	model.InitModelMap["ak"] = m
}

func initialAKModel() *AKModel {
	m := &AKModel{
	...
	for i := range m.inputs {
		...
		case 2:
			...
			t.Placeholder = "the key of the secret that stores the access key"
		case 3:
...
func (m *AKModel) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m *AKModel) Next() (model.Model, tea.Cmd) {
	...
	if m.focusIndex == len(m.inputs) {
		...
		err = k8s.CreateSecretStoreByAKSK(m.inputs[6].Value(), m.inputs[7].Value(), m.inputs[0].Value(),
			m.inputs[1].Value(), m.inputs[2].Value(), m.inputs[3].Value(),
			m.inputs[4].Value(), m.inputs[5].Value(), "", "")
		if err != nil {
			eModel.SetInfo(err.Error())
			eModel.SetPreModel(m)
			return eModel, nil
		}
		eModel.SetInfo(fmt.Sprintf("create SecretStore %s/%s success", m.inputs[7].Value(), m.inputs[6].Value()))
